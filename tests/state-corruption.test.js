...
describe("l'état reste utilisable après corruption", () => {
  ...
  it("lessons corrompu : le chargement reconstruit les 50 leçons", () => {
    poser({ ...REALISTIC, lessons: 42 });
    const s = State.load();
    expect(Object.keys(s.lessons)).toHaveLength(50);
  });
});
...
describe("items n'est JAMAIS persisté comme un tableau", () => {
  ...
  it("lessons non plus", () => {
    ...
    expect(Object.keys(relu.lessons)).toHaveLength(50);
  });
});
