...
describe("l'état reste utilisable après corruption", () => {
  ...
  it("lessons corrompu : le chargement reconstruit les 51 leçons", () => {
    poser({ ...REALISTIC, lessons: 42 });
    const s = State.load();
    expect(Object.keys(s.lessons)).toHaveLength(51);
  });
});
...
describe("items n'est JAMAIS persisté comme un tableau", () => {
  ...
  it("lessons non plus", () => {
    ...
    expect(Object.keys(relu.lessons)).toHaveLength(51);
  });
});
